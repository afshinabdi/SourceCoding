...
int CNonbinaryDSC::InitializeGraph(FACTOR_GRAPH &graph, uint q)
{
	...
	for (uint n = 0; n < graph.uiNumEdges; n++) {
	...
	for (uint n = 0; n < graph.uiNumCheckNodes; n++) {
		m_uiSparseEntry[n].clear();
		m_dtSparseValues[n].clear();
	}

	uint chkIdx, varIdx;
	for (uint n = 0; n < graph.edges.size(); n++) {
...
int CNonbinaryDSC::Decode(std::vector<DType> &s, std::vector<DType> &y, double adPx_y[],
	                       std::vector<DType> &dec_x, uint uiMaxIter)
{
