...
void CCheckNode::Initialize(uint q, CGaloisField *gf)
{
...
DType CCheckNode::ComputeMessage()
{
	DType a;
	int n;
...
void CVariableNode::Initialize(uint q, CGaloisField *gf)
{
	CNode::Initialize(q, gf);
...
void CVariableNode::SetInitialProbability(double adPx_y[], DType &y)
{
	// Px_y stored originally as P[x][y] = P[x*Q+y]
	for (DType a = 0; a < m_uiQ; a++)
		m_adLogP0[a] = log(adPx_y[a*m_uiQ + y]);
}

DType CVariableNode::ComputeMessage()
{
	double sum;
	...
	m_dtValue = std::distance(m_adTotalLL.begin(), it);

	// normalize messages to be probability distribution
	double dmax;
	for (auto edge : m_aEdges) {
		edge->dtDecValue = m_dtValue;
