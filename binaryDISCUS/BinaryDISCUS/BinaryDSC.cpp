...
uint CBinaryDSC::InitializeGraph(FACTOR_GRAPH &graph)
{
...
uint CBinaryDSC::Decode(std::vector<uint8_t> &s, std::vector<uint8_t> &y, 
					    double adPx_y[2][2], std::vector<uint8_t> &dec_x, uint uiMaxIter)
{
...
uint CBinaryDSC::Encode(std::vector<uint8_t> &x, std::vector<uint8_t> &s)
{
